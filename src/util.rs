...
pub fn set_socket_option<T>(fd: c_int, level: c_int, name: c_int, val: &T) -> io::Result<()> {
    let rv = unsafe {
        ...
        setsockopt(fd, level, name, val_ptr as *const c_void, size_of::<T>() as socklen_t)
    };
...
pub fn set_socket_option_mult<T>(fd: c_int, level: c_int, name: c_int, values: &[T]) -> io::Result<()> {
    let rv = if values.len() < 1 {
        ...
        unsafe {
            ...
            setsockopt(fd, level, name, val_ptr as *const c_void, (size_of::<T>() * values.len()) as socklen_t)
        }
    };
